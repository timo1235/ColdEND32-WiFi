...
#ifdef USE_WIFI
    fast_mode |= dataToControl.in_fast;
#endif
...
// All signals can be overwritten by each other
    ...
    #ifdef USE_WIFI
        if(wifi_mist_last_state != dataToControl.in_mist)
        {
            mist_stat = !mist_stat; // Toggle between mist on and off
            air_stat = LOW;         // switch off air stat
            wifi_mist_last_state = dataToControl.in_mist;
        }
        if(wifi_air_last_state != dataToControl.in_air)
        {
            air_stat = !air_stat; // Toggle between air on and off
            mist_stat = LOW;      // switch off mist stat
            wifi_air_last_state = dataToControl.in_air;
        }
    #endif
...
#ifdef USE_WIFI
    mist_stat |= dataToControl.in_mist;
    air_stat |= dataToControl.in_air;
#endif
#endif
}
...
void setValves()
{
    // Serial.print("Fast mode: " + String(fast_mode));
    // Serial.print(" Mist stat: " + String(mist_stat));
    // Serial.println(" Air stat: " + String(air_stat));
    // delay(500);
