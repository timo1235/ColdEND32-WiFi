...
DATA_COLDEND_TO_CLIENT dataToClient    = {};
...
esp_err_t PROTOCOL::sendMessageToClient(uint8_t *address, DATA_COLDEND_TO_CLIENT *data) { return esp_now_send(address, (uint8_t *) data, sizeof(DATA_COLDEND_TO_CLIENT)); }
...
void      PROTOCOL::onDataRecv(const uint8_t *address, const uint8_t *incomingData, int len) {
    ...
    memcpy(&dataToColdEnd, incomingData, sizeof(dataToColdEnd));
    // PROTOCOL::dumpDataToControl();
    if (dataToColdEnd.command.returnData) {
...
void PROTOCOL::dumpDataToControl() {
    ...
    Serial.println(dataToColdEnd.pot_mist);
    Serial.print("Pot Spit: \t\t");
    Serial.println(dataToColdEnd.pot_spit);
    Serial.print("In Mist: \t\t");
    Serial.println(dataToColdEnd.in_mist);
    Serial.print("In Fast: \t\t");
    Serial.println(dataToColdEnd.in_fast);
    Serial.print("In Air: \t\t");
    Serial.println(dataToColdEnd.in_air);
}
